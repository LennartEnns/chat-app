...
with check (
  get_role_in_chatroom((select auth.uid()), chatroom_id) in ('member', 'mod', 'admin')
);
...
using (
  ...
  and get_role_in_chatroom((select auth.uid()), chatroom_id) in ('member', 'mod', 'admin')
);
...
using (
  (select auth.uid()) = user_id
  and get_role_in_chatroom((select auth.uid()), chatroom_id) is not null
);
