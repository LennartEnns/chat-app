<template>
  <div>
        ...
        <UCommandPalette
          ...
          <template #item-trailing="{ index }">
            <div class="flex flex-row items-center justify-end">
              <UIcon :name="chatroomRolesVis[allGroups[index]?.current_user_role ?? 'viewer'].icon" class="mr-2" />
              <div class="text-muted">
                {{ chatroomRolesVis[allGroups[index]?.current_user_role ?? 'viewer'].label }}
              </div>
            </div>
          </template>
        </UCommandPalette>
    ...
    <div v-if="!selectedGroup" class="text-muted border-1 border-accented rounded-lg p-2 border-t-0 rounded-t-none">
      Select a group
    </div>
    <div
      v-else
    ...
    >
      ...
      >
        ...
        <div class="flex flex-row items-center gap-1">
          <UIcon :name="chatroomRolesVis[selectedGroup.current_user_role ?? 'viewer'].icon" />
...
const groupSearchItems = computed<CommandPaletteItem[]>(() => allGroups.value
