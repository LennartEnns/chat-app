<template>
  ...
  >
    ...
    >
      ...
      <UButton class="button" type="submit"> Sign In </UButton>
    </UForm>
...
const usingUsernameLogin = computed(
  () => !agnosticLoginState.usernameOrEmail?.includes("@")
);
  ...
  if (!usingUsernameLogin.value) {
    ...
    if (!error) {
      ...
      operationFeedbackHandler.displayError(
        getAuthErrorMessage(error, unknownErrorMessage)
      );
    }
  } else {
    // Login with username => Invoke edge function
    ...
    if (!error) {
      ...
      if (!setSessionError) {
        ...
        operationFeedbackHandler.displayError(
          getAuthErrorMessage(setSessionError, unknownErrorMessage)
        );
      }
    } else {
      console.log(
        `Error calling the username login function: ${JSON.stringify(error)}`
      );
      let description = unknownErrorMessage;
      if (error instanceof FunctionsHttpError) {
        const errorBody = await error.context.json();
        description = errorBody.code
          ? getAuthErrorMessage(errorBody)
          : errorBody.message ?? unknownErrorMessage;
      }
